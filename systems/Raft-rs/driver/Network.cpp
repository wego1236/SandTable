...
map<Node, int>::iterator Network::find_node(const int id) {
    map<Node, int>::iterator it;
    auto is_eq = [id](const pair<Node, int> &i){ return i.first.id == id; };
    it = std::find_if(peers.begin(), peers.end(), is_eq);
    return it;
}
