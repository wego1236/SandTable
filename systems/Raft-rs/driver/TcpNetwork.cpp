...
TcpNetwork::TcpNetwork(bool run_accept): Network() {
    ...
    // 绑定到本节点的 listening port 然后等待其他节点来连接即可
...
//    auto it = peers.find(peer);
    ...
    *(uint32_t *)&to_send.front() = size;
    ...
    // cerr_verbose<<"in send_to to_send"<< to_send.c_str()<<endl;
...
bool TcpNetwork::is_all_connected() {
    return all_of(peers.cbegin(), peers.cend(), [](const pair<Node, int>&i) {
...
// Library functions
extern "C" {
    ssize_t send_to(int node_id, const char* data,  int length) ;
    ssize_t recv_from(int node_id, string &data);
}
...
ssize_t send_to(int node_id, const char* data, int length) {
    ...
    // cerr_verbose<<"in client tcpnetwork send to str"<< str<<endl;
    auto it = net->find_node(node_id);
    return net->send_to(it->first, str);
}


ssize_t recv_from(int node_id, string &data) {
    auto it = net->find_node(node_id);
    return net->recv_from(it->first, data);
}
